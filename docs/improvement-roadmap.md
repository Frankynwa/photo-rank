...
> **⚠️ 文档状态提示（2026-08-19）**：本文档写于早期架构（五层流水线）时期，部分内容已过时或已落地，阅读时请注意：
>
> - **已落地**：L3 升级 TOPIQ-IAA、checkpoint/增量模式、断点续跑（见对应提交）
> - **已作废**：所有涉及 `core/layer5_analysis.py` / Layer 5 的条目（该层已于 v2.0.0 移除，分类标签并入 L3 VLM 评分）
> - **需重新核对**：下方"不可变约束"中的上传白名单描述（现已支持视频与 HEIC，白名单已扩展）；NIMA 相关对比基准（已被 TOPIQ+VLM 取代）
