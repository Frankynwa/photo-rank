...
def to_face_prompt_summary(r: Layer4Result) -> str:
    ...
    # 新分档：≥0.15 清晰；<0.15 只报"指标偏低"中性提示，由 VLM 依据画面自行判断。
    # 负面定性（"轻微模糊"/"明显模糊"）已移除：Laplacian 方差对墨镜（深色均匀区域）、
    # 侧脸（bbox 小偏暗）、纹理误检天然失真，负面文案会诱导 VLM 误报"人脸模糊"
    # 并连带触发钳制（实测饮品照奶泡误检假脸被钳到 2.63）。
    # 钳制侧双证据（L4 clarity<0.10 且 VLM 自报人脸模糊）见 layer3 _decide_vlm_clamp。
    # 环境人像（人脸占比<10%）：人脸小、Laplacian 方差天然偏低，clarity 低不代表"模糊"，
    # 不输出任何清晰度定性，避免诱导 VLM 误把"人脸模糊"当成影响主体清晰度的因素
